...
abstract class TweetSet {
  ...
  def ascendingByRetweet: Trending = ascendingByRetweet0(new EmptyTrending, this)
  def ascendingByRetweet0(accumulated: Trending, searchOn: TweetSet): Trending = { if (searchOn.isEmpty) { accumulated } else { ascendingByRetweet0(accumulated + searchOn.findMin, (searchOn.remove(searchOn.findMin))) } }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filter0(p: Tweet => Boolean, accu: TweetSet): TweetSet = {
    if (p(elem)) { (left.filter0(p, accu.incl(elem))).union(right.filter0(p, accu.incl(elem))) }
    else { (left.filter0(p, accu)).union(right.filter0(p, accu)) }
  }

  def union(that: TweetSet): TweetSet = { ((this.tail).union(that.incl(head))) }
...
object GoogleVsApple {
  ...
  val readIn = TweetReader.allTweets;

  val googleTweets: TweetSet = { readIn.filter((x: Tweet) => google.exists((j: String) => x.text.contains(j))) }

  val appleTweets: TweetSet = { readIn.filter((x: Tweet) => apple.exists((j: String) => x.text.contains(j))) }
